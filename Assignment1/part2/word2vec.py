...
def negSamplingLossAndGradient(
   ...
   ### Please use your implementation of sigmoid in here.
    un, idx, dup = np.unique(indices, return_index=True, return_counts=True)
    org = outsideVectors[un]
    dup[idx==0] *= -1
    org[idx!=0] *= -1
    
    z= sigmoid(np.dot(centerWordVec, org.T))

    loss = -(np.abs(dup) * np.log(z)).sum()
    gradCenterVec = np.dot(np.abs(dup) * (1 - z), -org)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    addd = np.outer(1 - z, centerWordVec)
    gradOutsideVecs[un] = dup[:, None] *addd    
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        cur_loss, grad_center, grad_outside = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += cur_loss
